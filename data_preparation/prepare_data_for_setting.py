...
def newcompound_newprotein():
    for i in range(5):
        input_file = r'C:\Users\DMIS_Quang\Desktop\project\dataset\kiba_davis_deeppurpose\binddingdbpkd.csv'
        train_file = r'C:\Users\DMIS_Quang\Desktop\project\dataset\kiba_davis_deeppurpose\%sbindingdb_train_newnew.csv'%i
        test_file = r'C:\Users\DMIS_Quang\Desktop\project\dataset\kiba_davis_deeppurpose\%sbindingdb_test_newnew.csv'%i
        index_compound = 0
        index_protein = 0
        with open(input_file) as csv_file_a:
            with open(train_file, mode='w',newline='') as result:
    ...
    return None
def make_val_set():
    path = r'C:\Users\DMIS_Quang\Desktop\project\dataset\kiba_davis_deeppurpose\5_folds_check\crossdomain\sparsity_metz_davis'
    list_files = os.listdir(path)
    # print(list_files[1][7:7+5])
    for file in list_files:
        if file.endswith('metz_data.csv'):
            input_file = os.path.join(path, file)
            output_file = os.path.join(path, file[0:5]+'_val_newnew.csv')
            output_file1 = os.path.join(path, file[0:5]+'_1_train_newnew.csv')
            print(output_file)
            index = 0
            with open(input_file) as csv_file_a:
